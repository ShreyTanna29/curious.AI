...
export async function POST(req: Request) {
  try {
    ...
    const systemPrompt =
      ...
      "Make sure to properly escape all double quotes within the code content using backslashes. " + "also always make sure to teminate strings, do not leave unterminated strings in JSON, your sent JSON should be parsed by JSON parser." +
      "Always include a package.json in root folder and mention a dev script to run the app.If app is not created using bundler or framework, then use http-server to start server. send the package.json file first and then send rest of the files" +
